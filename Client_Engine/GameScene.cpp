...
shap* InitShap(string smodpath)
{
	...
	wstring s= StoWs(smodpath);
    // 装载.x文件，初始化shap的mesh和dwNumMaterials
	if( FAILED( D3DXLoadMeshFromX((LPTSTR)(LPCTSTR)s.c_str(), D3DXMESH_SYSTEMMEM,
                                   g_pd3dDevice, NULL,
...
HRESULT InitGeometry()
{
    ...
    if( FAILED( D3DXLoadMeshFromX((LPTSTR)(LPCTSTR)L"model\\Tiger.x", D3DXMESH_SYSTEMMEM,
                                   g_pd3dDevice, NULL,
